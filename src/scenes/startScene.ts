...
export function startScene(k: GameContext) {
	...
	const title = k.add([k.text("PLANET SHOOTER", { size: titleSize }), k.pos(0, 0)]);

	const titleWidth = title.text.length * (titleSize * 0.6);
	title.pos.x = (k.width() - titleWidth) / 2;
	title.pos.y = k.height() / 2 - 60;

	const startButton = k.add([k.text("Start", { size: startButtonSize }), k.area({ cursor: "pointer" }), k.pos(0, 0)]);
